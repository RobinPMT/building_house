        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header border-bottom p-1">
                    <div class="head-label"><h6 class="mb-0">Câu hỏi thường gặp</h6></div>
                ...
                <table id="example" class="table table-bordered  display nowrap" cellspacing="0" width="100%">
                    ...
                    <tbody>
                        @if(isset($data, $status) && $status)
                            @foreach($data as $stt => $question)
                                <tr id="sid{{$question['id']}}">
                                    <td scope="row">{{$stt + 1}}</td>
                                    <td>{{$question['title']}}</td>
                                    <td style="">{{$question['creator']['name']}}</td>
                                    <td style="">
                                        <a class="badge badge-pill {{$question['arr_active']['class']}}" href="{{route('admin.get.action.question', ['active', $question['id']])}}">
                                            {{$question['arr_active']['name']}}
                                        </a>
                                    </td>
                                    <td style="display: none;">
                                        <a href="{{route('admin.get.action.question', ['delete', $question['id']])}}" class="delete-record"  id="confirm-color" data-id="{{$question['id']}}">
                                            <svg
                                        ...
                                        <a href="{{route('admin.get.find.question', $question['id'])}}" data-url-update="{{route('admin.update.question', $question['id'])}}" class="item-edit" data-toggle="modal" data-target="#modals-slide-in">
                            ...
                            <ul class="pagination" style="justify-content: flex-end">
                                @if(isset($meta['pagination']))
                                    <li class="paginate_button page-item previous {{$meta['pagination']['page'] > 1 ? '' : 'disabled'}}" id="example_previous">
                                        <a href="{{route('admin.get.list.question', ['_page' => $meta['pagination']['page'] - 1])}}" aria-controls="example" data-dt-idx="0" tabindex="0" class="page-link">Trang trước</a>
                                    </li>
                                    @if(isset($meta['pagination']['lastPage']))
                                        @for($i = 1; $i <= $meta['pagination']['lastPage']; $i++)
                                            <li class="paginate_button page-item {{$meta['pagination']['page'] == $i ? 'active' : ''}}">
                                                <a href="{{route('admin.get.list.question', ['_page' => $i])}}" aria-controls="example" data-dt-idx="1" tabindex="0" class="page-link">{{$i}}</a>
                                            </li>
                                        @endfor
                                    @endif
                                    <li class="paginate_button page-item next {{$meta['pagination']['page'] < $meta['pagination']['lastPage'] ? '' : 'disabled'}}" id="example_next">
                                        <a href="{{route('admin.get.list.question', ['_page' => $meta['pagination']['page'] + 1])}}" aria-controls="example" data-dt-idx="2" tabindex="0" class="page-link">Trang sau</a>
                                    </li>
                                @endif
                            </ul>
                        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            $(".item-add").click(function (event) {
                if($('#form-crud').attr('action') !== '{{route('admin.store.question')}}'){
                ...
                $('#form-crud').attr('action', '{{route('admin.store.question')}}');
