        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header border-bottom p-1">
                    <div class="head-label"><h6 class="mb-0">Bộ lọc diện tích</h6></div>
                ...
                <table id="example" class="table table-bordered  display nowrap" cellspacing="0" width="100%">
                    ...
                    <tbody>
                        @if(isset($data, $status) && $status)
                            @foreach($data as $stt => $filter)
                                <tr id="sid{{$filter['id']}}">
                                    <td scope="row">{{$filter['order']}}</td>
                                    <td>{{$filter['title']}}</td>
                                    <td style="">
                                        <a class="badge badge-pill {{$filter['arr_active']['class']}}" href="{{route('admin.get.action.filter', ['active', $filter['id']])}}">
                                            {{$filter['arr_active']['name']}}
                                        </a>
                                    </td>
                                    <td style="display: none;">
                                        <a href="{{route('admin.get.action.filter', ['delete', $filter['id']])}}" class="delete-record"  id="confirm-color" data-id="{{$filter['id']}}">
                                            <svg
                                        ...
                                        <a href="{{route('admin.get.find.filter', $filter['id'])}}" data-url-update="{{route('admin.update.filter', $filter['id'])}}" class="item-edit" data-toggle="modal" data-target="#modals-slide-in">
                            ...
                            <ul class="pagination" style="justify-content: flex-end">
                                @if(isset($meta['pagination']))
                                    <li class="paginate_button page-item previous {{$meta['pagination']['page'] > 1 ? '' : 'disabled'}}" id="example_previous">
                                        <a href="{{route('admin.get.list.filter', ['_page' => $meta['pagination']['page'] - 1])}}" aria-controls="example" data-dt-idx="0" tabindex="0" class="page-link">Trang trước</a>
                                    </li>
                                    @if(isset($meta['pagination']['lastPage']))
                                        @for($i = 1; $i <= $meta['pagination']['lastPage']; $i++)
                                            <li class="paginate_button page-item {{$meta['pagination']['page'] == $i ? 'active' : ''}}">
                                                <a href="{{route('admin.get.list.filter', ['_page' => $i])}}" aria-controls="example" data-dt-idx="1" tabindex="0" class="page-link">{{$i}}</a>
                                            </li>
                                        @endfor
                                    @endif
                                    <li class="paginate_button page-item next {{$meta['pagination']['page'] < $meta['pagination']['lastPage'] ? '' : 'disabled'}}" id="example_next">
                                        <a href="{{route('admin.get.list.filter', ['_page' => $meta['pagination']['page'] + 1])}}" aria-controls="example" data-dt-idx="2" tabindex="0" class="page-link">Trang sau</a>
                                    </li>
                                @endif
                            </ul>
                        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            $(".item-add").click(function (event) {
                if($('#form-crud').attr('action') !== '{{route('admin.store.filter')}}'){
                ...
                $('#form-crud').attr('action', '{{route('admin.store.filter')}}');
            ...
            if (newUserForm.length) {
                newUserForm.validate({
                    ...
                    rules: {
                        ...
                        // to: {
                        //     min: function() {
                        //         return parseInt($('#from').val()) > 0 ? parseInt($('#from').val()) : 0;
                        //     }
                        // },
                    },
                    messages: {
